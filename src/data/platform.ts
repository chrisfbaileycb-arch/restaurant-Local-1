...
// ============================================================
// Station health: the live equipment monitor on the owner dashboard.
// Devices are re-verified all day. If something critical drops, the
// alert fires and order entry is held until it is back.
// ============================================================

export type DeviceSeverity = 'blocking' | 'warn' | 'info';

/** How bad it is when this device stops answering. */
...
export const SEVERITY_COPY: Record<DeviceSeverity, { label: string; blurb: string }> = {
  blocking: {
    label: 'Holds orders',
    blurb: 'Food would be rung with nowhere to cook it or no way to charge for it, so new orders are held until this answers again.',
  },
  warn: {
    label: 'Warns only',
    blurb: 'You can keep ringing. The alert stays on screen until it is fixed or you acknowledge it.',
  },
  info: {
    label: 'Backup gear',
    blurb: 'A fallback tool. Its status is tracked so you know what you can reach for if something bigger drops.',
  },
};

/** Heartbeat settings for the monitor. */
export const HEALTH_CHECK = {
  intervalSeconds: 45,
  sweepStagger: 220,
  label: 'Every 45 seconds, open to close',
};

/** What the monitor actually does, in plain English (dashboard + marketing). */
export const HEALTH_RULES = [
  'Every paired device is pinged on a heartbeat all day — not just when you open the app.',
  'Two missed heartbeats and the device flips to Not connected with a time stamp of the last good answer.',
  'If a kitchen printer, kitchen display or card reader drops, order entry is held on every station at once.',
  'Nothing already fired is lost — open tickets, tabs and the queue stay exactly where they were.',
  'Fix it and hit Verify now: the block lifts the second the device answers, and the held orders release.',
];

/** Copy shown on the blocking alert, keyed by the device that dropped. */
export const BLOCK_REASONS: Partial<Record<DeviceKindId, string>> = {
  'kitchen-printer': 'Tickets would stack up on a printer nobody can read. Order entry is held so the line does not fall behind.',
  'kds': 'The kitchen rail is dark — new tickets would never appear on the screen. Order entry is held.',
  'card-reader': 'No reader means no card can be taken on this station. Order entry is held so nothing is rung that cannot be paid.',
  'receipt-printer': 'Guests would leave without a slip. Fix it or acknowledge to keep ringing.',
  'cash-drawer': 'The drawer is not answering — cash tenders cannot open it.',
};
